...
<body>
    ...
    <div class="row no-gutters">
       ...
       <?php
            ...
            $id = $_GET["id"];
            $query =mysqli_query ($koneksi, "SELECT * FROM panduan_ppdb WHERE id_panduan='$id'");
            $result = mysqli_fetch_array($query);
        ?>

        <div class="col-md-10 p-5 pt-2">
            <h3><i class="fas fa-calendar-plus mr-2 mb-3"></i> Edit panduan ppdb</a></h3> <hr class="mb-4">
            <form method="post">
                <div class="form-group">
                    <label>panduan ppdb</label>
                        <input type="text" name="panduan_ppdb" value="<?= $result['panduan_ppdb']; ?>" class="form-control">
                </div>
    ...
    <?php
        ...
        $query = "UPDATE panduan_ppdb SET panduan_ppdb = '$panduan_ppdb' WHERE id_panduan='$id' ";
        $result = mysqli_query($koneksi,$query);

        if (!$result) {
                    echo "<script>
                        alert('gagal mengedit panduan PPDB');
                        window.location.assign('edit_panduan.php');
                    </script>";
                } else {
                    echo "<script>
                        alert('panduan ppdb berhasil diedit!!!');
