...
<body>
    ...
    <div class="row no-gutters">
       ...
       <?php
        ...
        <div class="col-md-10 p-5 pt-2">
            <h3><i class="fas fa-calendar-plus mr-2 mb-3"></i> Edit Jadwal</a></h3> <hr class="mb-4">
            <form method="post">
                <div class="form-group">
                    <label>kegiatan</label>
                        <input type="text" name="kegiatan" value="<?= $result['kegiatan']; ?>" class="form-control">
                </div>
                <div class="form-group">
                    <label>tanggal</label>
                        <input type="text" name="tanggal" value="<?= $result['tanggal']; ?>" class="form-control">
                </div>
                <div class="form-group">
                    <label>waktu</label>
                        <input type="text" name="waktu" value="<?= $result['waktu']; ?>" class="form-control">
                </div>
                <div class="form-group">
                    <label>lokasi</label>
                        <input type="text" name="lokasi" value="<?= $result['lokasi']; ?>" class="form-control">
                </div>
                <button class="btn btn-danger"><a href="../manajemen_jadwal.php" style="text-decoration: none; color: #fff">Kembali</a></button>
    ...
    <?php
        ...
        $query = "UPDATE jadwal SET kegiatan = '$kegiatan', tanggal = '$tanggal', waktu = '$waktu', lokasi = '$lokasi' WHERE id_jadwal='$id' ";
        $result = mysqli_query($koneksi,$query);

        if (!$result) {
                    echo "<script>
                        alert('gagal mengedit jadwal');
                        window.location.assign('edit_jadwal.php');
                    </script>";
                } else {
                    echo "<script>
