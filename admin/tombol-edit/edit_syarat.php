...
<body>
    ...
    <div class="row no-gutters">
       ...
       <?php
            ...
            $id = $_GET["id"];
            $query =mysqli_query ($koneksi, "SELECT * FROM syarat_daftar WHERE id_syarat='$id'");
            $result = mysqli_fetch_array($query);
        ?>

        <div class="col-md-10 p-5 pt-2">
            <h3><i class="fas fa-calendar-plus mr-2 mb-3"></i> Edit Syarat Daftar</a></h3> <hr class="mb-4">
            <form method="post">
                <div class="form-group">
                    <label>Syarat Daftar</label>
                        <input type="text" name="syarat_daftar" value="<?= $result['syarat_daftar']; ?>" class="form-control">
                </div>
    ...
    <?php
        ...
        $query = "UPDATE syarat_daftar SET syarat_daftar = '$syarat_daftar' WHERE id_syarat='$id' ";
        $result = mysqli_query($koneksi,$query);

        if (!$result) {
                    echo "<script>
                        alert('gagal mengedit syarat daftar');
                        window.location.assign('edit_syarat.php');
                    </script>";
                } else {
                    echo "<script>
                        alert('Syarat Daftar berhasil diedit!!!');
